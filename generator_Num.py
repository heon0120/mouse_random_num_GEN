...
class RandomNumberCollector:
    def __init__(self, total_bytes):
        ...
        self.file_name = f"{time.time()}.txt"
    ...
    def on_move(self, x, y):
        ...
        digit = random_number % 10  # 0~9 사이의 값만 유지
        self.random_numbers.append(digit)
    ...
    def save_random_numbers(self):
        try:
            with open(self.file_name, 'w') as f:
                f.write(''.join(map(str, self.random_numbers)))
...
def main():
    try:
        total_bytes = int(input("생성할 난수 크기(바이트): "))
    except ValueError:
        print("정수만 입력 가능합니다.")
        sys.exit()
